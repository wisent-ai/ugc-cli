...
rm -f "$WISENT_OUTPUT_DIR/ugc-cli"
export CARGO_INCREMENTAL=0
export SOURCE_DATE_EPOCH=1
export CARGO_TARGET_DIR="$WISENT_OUTPUT_DIR/cargo-target-$WISENT_PLATFORM"
cd "$WISENT_SOURCE_DIR"
cargo build --release --locked --manifest-path "$WISENT_SOURCE_DIR/Cargo.toml" --bin ugc-cli
install -m 0755 "$CARGO_TARGET_DIR/release/ugc-cli" "$WISENT_OUTPUT_DIR/ugc-cli"
